...
addEventListener("DOMContentLoaded", (event) => {
    // Load DJs
    let djIndex = djs.findIndex((dj) => dj.name === loggedInDJ);
    selectedDJ = loggedInDJ;
    let timelist = document.getElementById("listoftimes");
    timelist.innerHTML = "";
    for(let j = 0; j < djs[djIndex].times.length; j++) {
        let tempLI = document.createElement("li");
        tempLI.appendChild(document.createTextNode(djs[djIndex].times[j].timeslot));
        ...
        tempLI.addEventListener("click", function(e) {
            if(e.target) {
                ...
                /* populate playlist */
                populatePlaylist(loggedInDJ, selectedTime, djs[djIndex].times[j].songs);
            }
        });
    }
